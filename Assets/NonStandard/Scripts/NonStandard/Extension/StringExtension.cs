...
namespace NonStandard.Extension {
	public static class StringExtension {
		...
		public static int IndexOfFirstCheckingBackward(this string str, IEnumerable<string> delimiter, out string firstDelimiter, int startIndex = 0, int limitIndex = -1) {
			if (limitIndex < 0) { limitIndex = str.Length; }
			for (int i = limitIndex-1; i >= startIndex; --i) {
				foreach(string s in delimiter) {
					int lenPossible = limitIndex - i;
					if (lenPossible < s.Length) { continue; } // skip this word if it's too long for the space to check
					if (str.IsSubstringAt(s, i)) {
						firstDelimiter = s;
						return i;
					}
				}
		...
		public static string EndsWith(string haystack, IEnumerable<string> needles, int limitIndex = -1) {
			if (limitIndex < 0) { limitIndex = haystack.Length; }
			foreach(string s in needles) {
				if (IsSubstringAt(haystack, s, limitIndex - s.Length)) {
					return s;
				}
			}
			return null;
		...
		public static string ForEachBetween(this string str, Func<string, string> manipulateSubstring, string delimiter = "\n") {
			IList<string> lines = str.Split(delimiter);
			StringBuilder sb = new StringBuilder();
